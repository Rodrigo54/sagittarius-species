...
/** Envia um arquivo local (ex.: referência de pose do ControlNet) pro
 * `input/` do ComfyUI via `/upload/image`, em vez de assumir o caminho de
 * instalação no disco — mesma ideia de não hardcodar a pasta do ComfyUI uma
 * segunda vez além do `BASE_URL`. Devolve o nome final que o `LoadImage`
 * deve referenciar (o ComfyUI pode renomear em caso de colisão). */
