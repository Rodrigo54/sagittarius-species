...
/** Confere que, pra todo gênero em que `geracaoArt.<gênero>` está presente,
 * a contagem de `variantes` bate exatamente com `counts.<gênero>` e as
 * chaves são sequenciais "001".."NNN", zero-padded, sem buraco — validação
 * cruzada entre `counts` (nível raiz) e `geracaoArt` (aninhado), impossível
 * de expressar só com `z.record`/enum isolado. Substitui
 * `validarChavesVariantes` de `generate-art/validation.ts`. */
