...
**Quem enfileira geração no ComfyUI é sempre o usuário, nunca esta skill** (regra do `CLAUDE.md`): gerar
...
- **Enfileirar geração no ComfyUI, em qualquer volume** — amostra, lote ou variante avulsa, com ou sem `-s`.
  Regra do `CLAUDE.md`: quem roda `bun run art` é sempre o usuário. A skill só executa `-e/--export-prompt` (não
